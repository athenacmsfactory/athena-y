  ...
  return (
    <article className={`jet-card ${jet.origin?.toLowerCase() || ''} flex flex-col bg-white rounded-xl overflow-hidden shadow-lg transition-all duration-300 hover:shadow-2xl hover:-translate-y-1`}>
      <div className="relative aspect-video overflow-hidden">
        <EditableMedia 
          src={jet.image_url} 
          cmsBind={{ file: 'jets', index: index, key: 'image_url' }}
        ...
        <div className="absolute top-4 right-4 bg-sky-500 text-white px-3 py-1 rounded-full text-sm font-bold shadow-md">
          <span data-dock-type="text" data-dock-bind={`jets.${index}.introduction_year`}>{jet.introduction_year}</span>
        </div>
      </div>
      ...
      <div className="p-6 flex-grow flex flex-col">
        ...
        <h3 className="text-xl font-bold text-slate-900 mb-1">
          <span data-dock-type="text" data-dock-bind={`jets.${index}.name`}>{jet.name}</span>
        </h3>
        
        <p className="text-sm text-slate-500 italic mb-4">
          <span data-dock-type="text" data-dock-bind={`jets.${index}.full_name`}>{jet.full_name}</span>
        </p>
        
        <p className="text-slate-600 text-sm leading-relaxed line-clamp-4">
          <span data-dock-type="text" data-dock-bind={`jets.${index}.description`}>{jet.description}</span>
        </p>
      </div>
    </article>
  );
